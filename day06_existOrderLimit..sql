...
==============================================================================*/
 ...
 where exists (select urun_isim from mart m where n.urun_isim=m.urun_isim);
  ...
  /* -----------------------------------------------------------------------------2'nin tersi bir sorgu
  ORNEK3: Her iki ayda birden satılan ürünlerin URUN_ISIM'lerini ve bu ürünleri
  NİSAN ayında satın alan MUSTERI_ISIM'lerinin disindakileri listeleyen bir sorgu yazınız. 
 -----------------------------------------------------------------------------*/
 select urun_isim, musteri_isim from nisan as n
        ...
        update insanlar set isim=null where isim='NO NAME';
         select * from insanlar;
           /* -------------------------------------------------------------
            ...
            gibi ifade yazmamak için. coalesce=birleşmek null lari topla
                 ---------------------------------------------------------------*/
                 update insanlar 
...
select * from kisiler 
where soyisim='Bulut'  
order by 5; -- 5 sutun sayisi gore siraladik
/* ----------------------------------------------------------------------------*/
--              LIMIT             --
...
select * from kisiler limit 10;
 ...
 select * from kisiler limit 10,2; -- 10 dahil degil sonraki 2 elemani yaz
select * from kisiler where id>5 limit 2;
select * from kisiler where id>5; -- kac adet siniri yoksa bu da olur.
/* ----------------------------------------------------------------------------
...
-- select * from kisiler  order by maas desc limit 3;fetch next 3 rows only         oracle cozumu

/* ----------------------------------------------------------------------------
  ORNEK1: MAAŞ'a gore siralamada , 4, 5, ve 6, kişilerin bilgilerini listeleyen sorguyu yazınız.
-----------------------------------------------------------------------------*/
