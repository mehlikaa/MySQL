...
-- in yerine = olmaz , cunku = sadece bir adet veri icin kullanilabilir.
/* ----------------------------------------------------------------------------
...
==============================================================================*/   
 ...
 -----------------------------------------------------------------------------*/
  select marka_isim, calisan_sayisi,  (select SUM(maas) from calisanlar where marka_isim=isyeri) toplam_maas
  from markalar; -- as kullanilmayabilir
    select marka_isim, calisan_sayisi,  (select SUM(maas) from calisanlar where marka_isim=isyeri) as toplam_maas
  from markalar; --   as kullanilarak
  
  /* -----------------------------------------------------------------------------
  ORNEK5: Her markanin ismini, calisan sayisini ve 
  o markaya ait calisanlarin ortalama maaşini listeleyen bir Sorgu yaziniz.
 -----------------------------------------------------------------------------*/
 select marka_isim, calisan_sayisi, 
 (select round(AVG(maas)) from calisanlar where marka_isim=isyeri) as  ortalama_maas
from markalar; 
 ...
 -----------------------------------------------------------------------------*/    
  SELECT marka_isim, calisan_sayisi,
    ...
    (SELECT MAX(maas) FROM calisanlar WHERE marka_isim = isyeri) AS min_maas FROM markalar; 
