...
from manav
group by urun_adi
having max(urun_miktari)!=min(urun_miktari)		-- != yerine <> de kullanilabilir
order by max(urun_miktari);									-- having not max(urun_miktari)=min(urun_miktari) bu da oluyor hocam
 /*  select urun_adi, max(urun_miktari) maks
from manav
group by urun_adi
...
-----------------------------------------------------------------------------*/   
 select count(distinct urun_adi) KacUrunAdi  from manav;
 
 /* -----------------------------------------------------------------------------
  ORNEK2: satılan meyve + isimlerin farklı olanlarını listeyen sorguyu yazınız.
------------------------------------------------------------------------------*/
select distinct urun_adi farkli_meyveler, isim -- farkli_meyveler olmayabilir sutun basligi
 from manav;
 /* ----------------------------------------------------------------------------
  ORNEK3: satılan meyvelerin urun_mikarlarinin benzersiz  olanlarının 
  toplamlarini listeyen sorguyu yazınız.
-----------------------------------------------------------------------------*/ 
select sum(distinct urun_miktari) farkli_miktarlar -- farkli_meyveler olmayabilir sutun basligi
 from manav;