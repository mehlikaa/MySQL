...
-----------------------------------------------------------------------------*/ 
SELECT 
    urun_adi, COUNT(isim) kisi_sayisi -- tabloya kisi sayisi ile yazilir. Yazilmazsa isim diye kendi yazar
FROM
    manav
WHERE
...
/* ----------------------------------------------------------------------------
  ...
  yazınız.
-----------------------------------------------------------------------------*/ 
select urun_adi, min(urun_miktari) minnn, max(urun_miktari) maks
from manav
...
/* ----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
 select urun_adi, isim,  sum(urun_miktari) Satilan_top_urun
from manav 
...
/* ----------------------------------------------------------------------------
 ...
 from manav
 group by isim, urun_adi
 having sum(urun_miktari)>=3		-- olay cok karisti . grouptan sonra where yerine having kullaniliyor
